document.addEventListener('DOMContentLoaded', function() {
		function slowScrollTo(target, duration = 800, offset = 80) {
	...
	const nextSection = document.querySelector('#projects');
		if (scrollBtn && nextSection) {
			scrollBtn.addEventListener('click', function(e) {
				e.preventDefault();
				slowScrollTo(nextSection, 800, 80); // 800ms, 80px offset
			});
		}
	...
	const navbarLinks = document.querySelectorAll('.navbar a[href^="#"]');
		navbarLinks.forEach(function(link) {
			link.addEventListener('click', function(e) {
				...
				if (targetSection) {
					e.preventDefault();
					slowScrollTo(targetSection, 800, 80); // 800ms, 80px offset
				}
			});
		});
});
